...
class functionObj:
    ...
    def __init__(self, name, start=0, end=0, added=0, deleted=0,exception_add=0,exception_del=0):
        self.name = name
        self.start = start
        self.end = end
...
def parseText(filename):
    ...
    for line in inf:
        ...
        if(phase == LOOKFORNAME):
            ...
            if(bracketDepth == nestingDepth and phase == LOOKFOREND):
                ...
                funcToAdd = functionObj(self.parseFunctionName(shortFunctionName), funcStart, funcEnd, total_add, total_del,etotal_add,etotal_del)
            ...
            bracketDepth -= line.count("}")
            bracketDepth += line.count("{")
            if(exceptionBracketDepth>bracketDepth or exceptionBracketDepth==0 and phase2==LOOKFOREXCPEND):
                phase2=LOOKFOREXCP
                exceptionBracketDepth=0
            ...
            if(bracketDepth == nestingDepth):
                ...
                funcToAdd = functionObj(parseFunctionName(shortFunctionName), funcStart, funcEnd, total_add, total_del,etotal_add,etotal_del)
    ...
    if(shortFunctionName != ""):
        ...
        funcEnd = lineNum
        funcToAdd = functionObj(parseFunctionName(shortFunctionName), funcStart, funcEnd, total_add, total_del)
        #Add assertions from current function
        #funcToAdd.assertionList += assertCurFunc
        functions.append(funcToAdd)
